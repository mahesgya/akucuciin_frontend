          ...
          <div className='font-["Montserrat"] text-sm border-l-2 border-neutral-200 pl-2 w-full'>
            {contents.map((content, index) => (
              <div className="flex flex-row items-center justify-between w-full pr-2 space-x-4" key={index}>
                ...
                <p className='w-full text-end text-xs mt-2 font-["Montserrat"] break-words font-semibold'>{content.value === "" ? "-" : content.value}</p>
                ...
                {content.phone && (
                  <a href={content.phone} className='w-full mt-2 font-["Montserrat"] text-end text-blue-600 hover:text-blue-800 underline hover:no-underline'>
                    {content.phone}
                  </a>
                )}
              </div>
