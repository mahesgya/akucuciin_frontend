...
const RatingSwal = (accessToken, orderId) => {
    ...
    `,
		...
		preConfirm: () => {
			...
			const selectedChips = [];
			document
				.querySelectorAll(".chip[data-selected='true']")
				.forEach((chip) => {
		...
		if (result.isConfirmed) {
			...
			// Submit the review
			try {
				await CustomerServices.postReview(
					accessToken,
					orderId,
					rating,
					comment
				...
				{isOpen && (
					<div className='font-["Montserrat"] text-sm border-l-2 border-neutral-200 pl-2 w-full'>
						{contents.map((content, index) => (
							<div
								className="flex flex-row items-center justify-between w-full pr-2 space-x-4"
								key={index}
							>
								<p className='text-xs mt-2 font-["Montserrat"] whitespace-nowrap font-semibold'>
									{content.field}
								</p>
								...
								{content.link && (
									<a
										...
										title={content.link}
										className='w-full mt-2 font-["Montserrat"] whitespace-nowrap overflow-hidden text-ellipsis text-blue-600 hover:text-blue-800 underline hover:no-underline transition-colors duration-200'
									>
								...
								{content.phone && (
									<a
										href={content.phone}
										className='w-full mt-2 font-["Montserrat"] text-end text-blue-600 hover:text-blue-800 underline hover:no-underline'
									>
										{content.phone}
									</a>
								)}
			...
			>
				...
				{isOpen && (
					<div className='font-["Montserrat"] text-sm border-l-2 border-neutral-200 pl-2 w-full'>
						{contents.map((content, index) => (
							<div
								className="flex flex-col items-start justify-between w-full pr-2"
								key={index}
							>
								<p className='text-sm mt-2 font-["Montserrat"] whitespace-nowrap font-semibold'>
									{content.field}
								</p>

								<p className='w-full text-sm text-justify mt-2 font-["Montserrat"]'>
									{content.value === "" ? "-" : content.value}
								</p>
								...
								{content.phone && (
									<a
										href={content.phone}
										className='w-full mt-2 font-["Montserrat"] text-blue-600 hover:text-blue-800 underline hover:no-underline'
									>
			...
			{orderPhases.map((phase, index) => (
				<div key={index} className="flex items-start relative mb-6">
					...
					{index !== orderPhases.length - 1 && (
						<span
							className={`absolute left-[8px] top-6 w-1 h-full ${
					...
					<div className="lg:text-sm text-xs flex flex-col w-full items-start justify-between space-y-3 mt-4">
						...
						<FlexRowAccordion
							...
							contents={[
								...
								{
									field: "Diskon Kupon",
									value: data.coupon?.multiplier
										? `${data.coupon.multiplier} %`
										: "-",
								},
