    ...
    <div className="lg:h-screen h-full flex flex-col items-center justify-start bg-gray-50">
      ...
      <div className="grid lg:grid-cols-3 grid-cols-1 gap-4 h-full pb-8 w-full px-4 lg:px-20">
        <div className="lg:order-1 order-3 flex flex-col bg-white p-6 rounded-lg shadow-lg items-center">
          <h2 className='lgtext-xl text-lg font-semibold font-["Montserrat"]'>Detail Order</h2>
          ...
          <div className="lg:text-sm text-xs flex flex-col w-full items-start justify-between space-y-3 mt-4">
            <div className="flex flex-row items-center justify-between w-full">
              <p className='font-["Montserrat"] font-semibold'>Paket Laundry </p>
              <p className='font-["Montserrat"] font-medium'>{data.package.name}</p>
            </div>
            <div className="flex flex-row items-center justify-between w-full">
              <p className='font-["Montserrat"] font-semibold'>Total </p>
              {data.price_after !== "0.00" && <p className='font-["Montserrat"] font-medium'>Rp {data.price_after}</p>}
              {data.price_after === "0.00" && <p className='font-["Montserrat"] font-medium'>Belum diinput oleh mitra</p>}
            </div>
            <div className="flex flex-row items-center justify-between w-full">
              <p className='font-["Montserrat"] font-semibold'>Berat </p>
              {data.weight === "0.00" && <p className='font-["Montserrat"] font-medium'>Belum diinput oleh mitra</p>}
              {data.weight !== "0.00" && <p className='font-["Montserrat"] font-medium'>{data.weight} kg</p>}
            </div>
            ...
            <div className="flex flex-row items-center justify-between w-full">
              <p className='font-["Montserrat"] font-semibold'>Tanggal Order </p>
            ...
            <div className="flex flex-row items-center justify-between w-full">
              <p className='font-["Montserrat"] font-semibold'>Tanggal Penjemputan </p>
              <p className='font-["Montserrat"] font-medium'>
                {ddmmyyFormatter(data.pickup_date.substring(data.pickup_date.lastIndexOf(" ") + 1), "id-ID")}
                {}
              </p>
            </div>

            <div className="flex flex-row items-center justify-between w-full">
              <p className='font-["Montserrat"] font-semibold'>Jam Penjemputan </p>
              <p className='font-["Montserrat"] font-medium'>{data.pickup_date.substring(0, data.pickup_date.lastIndexOf(" "))}</p>
            </div>
            ...
            <FlexRowAccordion
              ...
              contents={[
                ...
                {
                  field: "Diskon Kupon",
                  value: data.coupon?.multiplier ? `${data.coupon.multiplier} %` : "-",
                },
            ...
            <FlexRowAccordion
              ...
              contents={[
                ...
                {
                  field: "Lokasi",
                  value: data.laundry_partner.area + ", " + data.laundry_partner.city,
                },
                {
                  field: "Kontak",
                  phone: `https://wa.me/${transformPhoneNumber(data.laundry_partner.telephone)}`,
                },
              ]}
            />
          </div>
        ...
        <div className="order-2 flex flex-col h-full">
          <div className="flex flex-col h-full bg-white p-6 rounded-lg shadow-lg items-center justify-between">
            <div className="flex flex-col items-center justify-center w-full">
              <h2 className='lg:text-xl text-lg font-semibold font-["Montserrat]'>Track Order</h2>
          ...
          {data.status === "batal" && (
            <div className="flex flex-col space-y-4 bg-white p-6 rounded-lg shadow-lg items-center justify-center mt-4">
              <h2 className='text-lg font-semibold font-["Montserrat"]'>Order dicancel</h2>
              <p className='text-sm text-black font-["Montserrat"]'>{data.cancel_reason}</p>
            </div>
          )}
        </div>
          ...
          <div className="flex flex-col bg-white p-6 rounded-lg shadow-lg">
            <div className="flex flex-row items-center justify-between mb-4">
              <div className="flex flex-col space-y-2">
                <p className='font-semibold lg:text-xl text-sm font-["Montserrat"]'>{data.package.name}</p>
                <p className='font-semibold lg:text-sm text-xs text-neutral-400 font-["Montserrat"]'>{data.package.description}</p>
              </div>
            ...
            <div className="flex space-x-2 justify-between">
              <div className="flex flex-col space-y-2">
                {data.package.features.map((feature, index) => (
                  <div className="flex flex-row items-center align-middle space-x-2 mt-4" key={index}>
                    ...
                    <p key={index} className='font-semibold lg:text-sm text-xs text-neutral-400 font-["Montserrat"]'>
                      {feature}
                    </p>
                  </div>
                ))}
              ...
              <div className="flex flex-col-reverse space-y-2 mt-4">
                {data.status === "pending" && <div className="w-full font-quick px-3 py-1 lg:text-sm text-xs border border-black bg-yellow-100 text-black font-semibold rounded-lg">{data.status}</div>}
                {data.status === "batal" && <div className="w-full font-quick px-3 py-1 lg:text-sm text-xs border border-black bg-red-100 text-black font-semibold rounded-lg">{data.status}</div>}
                {data.status === "selesai" && <div className="w-full font-quick px-3 py-1 lg:text-sm text-xs border border-black bg-green-100 text-black font-semibold rounded-lg">{data.status}</div>}
                {(data.status === "penjemputan" || data.status === "pencucian" || data.status === "pengantaran") && (
                  <div className="w-full font-quick px-3 py-1 lg:text-sm text-xs border border-black bg-[#B3EBF2]/50 text-black font-semibold rounded-lg">diproses</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center font-['Montserrat'] mt-4 shadow-lg">
            {data.status === "selesai" && data.rating === 0 && (
              <div className="bg-white rounded-lg p-6 md:p-8 w-full">
                <h2 className="text-lg font-bold text-center text-black mb-2 font-['Montserrat']">Berikan rating yuk!</h2>
                <p className="lg: text-sm text-center text-gray-600 mb-6 font-['Montserrat'] font-medium">(1 mengecewakan, 5 mantap! )</p>
            ...
            {data.status === "selesai" && data.rating !== 0 && (
              <div className="bg-white rounded-lg p-6 md:p-8 w-full">
                <h2 className="text-lg font-bold text-center text-black mb-2 font-['Montserrat']">Terima kasih sudah memberikan penilaian ^^</h2>

                <h3 className="text-lg font-semibold text-center text-gray-600 mb-4 font-['Montserrat']">"{data.review}"</h3>
