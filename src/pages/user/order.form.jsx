        ...
        <div className="h-full flex flex-col w-full space-y-4">
          <div className="relative flex flex-col w-full bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-300/30 dark:border-neutral-700">
            <div className="flex flex-row items-center justify-between mb-4">
              <div className="flex flex-col space-y-2">
                ...
                <p className='font-medium text-sm text-neutral-600 dark:text-dark-text/70 font-["Montserrat"]'>{activePaket?.description}</p>
              </div>
              <div className="flex flex-col justify-center items-end space-y-2 h-full">
                <p className='font-medium text-base text-green-400 font-["Montserrat"]'>Rp{parseInt(activePaket?.price_text).toLocaleString("id-ID")}</p>
              </div>
            </div>
              ...
              <div className="flex flex-col space-y-2 h-full">
                {activePaket?.features?.map((feature, index) => (
                  <div className="flex flex-row items-center align-middle space-x-2 mt-4" key={index}>
                    ...
                    <p className='font-medium lg:text-sm text-xs text-gray-500/80 dark:text-dark-text/80 font-["Montserrat"]'>{feature}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="w-full bg-white dark:bg-dark-card rounded-xl px-6 py-5 shadow-sm border border-gray-300/30 dark:border-neutral-700">
            ...
            <input
              ...
              className="font-['Montserrat'] rounded-xl p-2 w-full bg-white dark:bg-dark-card border border-0.2 border-gray-500/30 dark:border-neutral-700 shadow-sm text-gray-700 dark:text-dark-text placeholder:text-gray-400 dark:placeholder:text-gray-400 focus:outline-none focus:border-b-2 text-base"
            />

            {lastOrder && lastOrder.maps_pinpoint && (
              <div className="mt-4 w-full flex flex-col gap-2 justify-between items-center mb-2">
                <div className="border border-white align-middle bg-[#687EFF] w-full rounded-xl">
                  <button type="button" onClick={() => setFormData((prev) => ({ ...prev, maps_pinpoint: lastOrder.maps_pinpoint }))} className="font-['Montserrat'] w-full p-2 rounded-xl font-semibold text-white text-sm">
                    Gunakan Maps Sebelumnya
                  </button>
                </div>

                <div className="border border-[#687EFF] w-full rounded-xl align-middle">
                  <button type="button" onClick={() => window.open(`${lastOrder.maps_pinpoint}`, "_blank")} className="font-['Montserrat'] w-full p-2 rounded-xl font-semibold text-[#687EFF] text-sm">
                    Cek Maps Sebelumnya
                  </button>
                </div>
              </div>
            ...
            <div className="flex flex-row space-x-1 items-center justify-start hover:underline">
              ...
              <a target="_blank" href="https://www.google.com/maps" rel="noopener noreferrer" className="font-['Montserrat'] font-medium text-[#687EFF] mb-1 mt-2 text-sm">
                Google Maps
              </a>
            </div>
          </div>
          ...
          <div className={`w-full rounded-xl shadow-sm ${formData.referral_code || formData.coupon_code ? "bg-gradient-to-r from-[#687EFF] to-[#39BCF8]" : "bg-white dark:bg-dark-card border border-gray-300/30 dark:border-neutral-700"}`}>
              ...
              <div className="space-y-2 bg-gradient-to-r from-[#687EFF] to-[#39BCF8] text-white font-bold py-4 px-8 rounded-xl mb-4">
                {formData.coupon_code && (
                  <div className="space-y-2">
                    <p className="font-['Montserrat'] font-bold text-base">Yay! Anda dapat promo!</p>
                    <div className="flex flex-col space-y-2">
                ...
                {!formData.coupon_code && formData.referral_code && <p className="font-['Montserrat'] font-bold text-base text-center w-full">Mantap! Sebarkan ke teman anda</p>}
                ...
                <div className="flex flex-row items-center justify-between space-x-8">
                  {formData.referral_code && (
                    <div className="rounded-xl text-sm font-semibold flex items-center justify-center py-2 px-8 bg-white text-[#687EFF] w-full">
                      <img src="/Images/profile.webp" alt="profile" className="h-6 w-6 mr-2" /> {formData.referral_code}
                    </div>
                  )}
                  <div onClick={() => handleVoucherReferralClick()} className="bg-transparent text-sm text-white font-['Montserrat'] border rounded-xl w-full px-4 py-2 font-semibold hover:bg-white/20 flex items-center justify-center">
                    Edit
                  </div>
                </div>
              </div>
            ...
            {formData.referral_code === "" && formData.coupon_code === "" && (
              <div>
                <div onClick={() => handleVoucherReferralClick()} className="px-10 py-5 flex flex-row align-middle items-center justify-center bg-gradient-to-r from-[#687EFF] to-[#39BCF8] text-white font-bold p-4 rounded-xl">
                ...
                <button
                  ...
                  onClick={() => window.open("https://www.instagram.com/akucuciin.id/", "_blank")}
                >
                  <div className="w-full flex flex-row h-full items-center align-middle justify-center text-[#687EFF] font-['Montserrat'] font-semibold text-sm lg:text-base hover:underline">
                    <p className="text-[#687EFF] mr-1 text-sm">cek promo lain di instagram akucuciin.id </p>
                    <img src="/Images/ArrowLeft2.png" alt="arrow" className="h-4 rotate-180" />
                  </div>
                </button>
              </div>
            )}
        ...
        <div className="bg-white dark:bg-dark-card flex flex-col rounded-xl px-6 md:px-10 py-5 space-y-4 h-fit w-full relative shadow-sm border border-gray-300/30 dark:border-neutral-700">
          ...
          <div className="flex flex-col w-full">
            <DatePicker
              ...
              className="hidden"
              dayClassName={() => "font-['Montserrat'] text-gray-800 dark:text-dark-text"}
              weekDayClassName={() => "font-['Montserrat'] text-gray-500 dark:text-dark-text/70"}
            />
            ...
            <div className="flex flex-col w-full h-full mt-4 border-t-2 border-gray-200 dark:border-neutral-700 pt-4">
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        value={slot.label}
                        checked={isSelected}
                        onChange={() => !isDisabled && setPickupHours(slot.label)}
                        disabled={isDisabled}
                        className={`form-radio h-4 w-4 ${isDisabled ? "text-gray-400" : "text-indigo-600 focus:ring-indigo-500"} border-gray-300 dark:border-neutral-700`}
                      />
                      <label htmlFor={`time-slot-${slot.id}`} className={`ml-2 font-["Montserrat"] text-sm lg:text-base font-semibold ${isDisabled ? "text-gray-400" : "text-gray-700 dark:text-dark-text"} cursor-pointer`}>
                        {slot.label}
                      </label>
                    </div>
                  );
              ...
              {formData.note ? (
                <div className="flex flex-col w-full h-fit space-y-2 mt-2 border-l-2 mb-4 md:mb-12 border-[#687EFF] pl-4">
                  <p className="font-['Montserrat'] max-w-60 h-full overflow-clip text-ellipsis font-medium text-[#7F7F7F] dark:text-dark-text/80 mb-1 text-sm">{formData.note}</p>
                  <div onClick={() => handleEditNotes()} className="flex w-fit items-center text-sm justify-between bg-white dark:bg-dark-card border border-[#687EFF] text-[#687EFF] font-semibold py-2 px-4 rounded-xl ">
                    Edit
                  </div>
                </div>
              ) : (
                <div onClick={() => handleEditNotes()} className="flex items-center text-sm mb-4 md:mb-12 justify-between mt-2 border bg-[#687EFF] font-semibold text-white border-gray-300 dark:border-neutral-700 py-2 px-4 rounded-xl w-fit">
                  <p>Catatan</p>
