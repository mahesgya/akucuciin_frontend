	...
	const handleValidationResponse = async (responses) => {
		...
		if (responses.coupon_code === "") {
			...
			if (!responses.couponResponse.success) {
				...
				const percentageDiscount = isSpecialVoucher(
					responses.couponResponse.data?.name
				)
					? "6% - 62"
					: responses.couponResponse.data.multiplier;
				await toastSuccess(
					`Kupon "${responses.couponResponse.data.name}" Valid dengan diskon ${percentageDiscount}%`
				);
			}
		}
	};
	...
	return (
		<div className="font-['Montserrat'] flex flex-col bg-[#F4F5FF] dark:bg-dark-bg h-full min-h-screen w-screen pb-8 dark:text-dark-text">
			...
			<form
				...
				className="lg:grid lg:grid-flow-col lg:grid-cols-2 space-y-4 lg:space-y-0 gap-8 w-full h-full px-3 lg:px-24"
			>
				<div className="h-full flex flex-col w-full space-y-4">
					...
					<div className="w-full bg-white dark:bg-dark-card rounded-xl px-6 py-5 shadow-sm border border-gray-300/30 dark:border-neutral-700">
						<h4 className="font-['Montserrat'] font-semibold text-left text-gray-700 dark:text-dark-text mb-1 text-base">
							Link Google Maps
						</h4>
						<input
							required
							...
							<div className="mt-4 w-full flex flex-col gap-2 justify-between items-center mb-2">
								<div className="border border-white align-middle bg-[#687EFF] w-full rounded-xl">
									<button
										...
										className="font-['Montserrat'] w-full p-2 rounded-xl font-semibold text-white text-sm"
									>
								...
								<div className="border border-[#687EFF] w-full rounded-xl align-middle">
									<button
										...
										className="font-['Montserrat'] w-full p-2 rounded-xl font-semibold text-[#687EFF] text-sm"
									>
										Cek Maps Sebelumnya
									</button>
								</div>
					...
					<Link
						...
						className="font-quick font-semibold text-center text-blue-600 dark:text-blue-400 mb-1 mt-4 text-md md:text-md hover:underline"
					>
